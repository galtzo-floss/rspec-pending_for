Project: rspec-pending_for — Development Guidelines (for advanced contributors)
...
- Ruby and Bundler
  - Runtime supports very old Rubies (>= 1.8.7) but development tooling targets >= 2.3 because of CI/setup-ruby and dev dependencies.
...
- Gem signing (for releases)
  ...
  - Important for local testing (to avoid hanging prompts): ALWAYS skip signing when building locally to test the packaging or install process. Without the private key password, the build will wait indefinitely at a signing prompt.
    - One-off commands (recommended):
      - SKIP_GEM_SIGNING=true gem build rspec-pending_for.gemspec
...
- Running tests (verified)
  ...
  - Focused runs
    ...
    - Example: K_SOUP_COV_MIN_HARD=false bin/rspec spec/rspec-pending_for/class_spec.rb:42
  - Output visibility
...
- Adding new tests (guidelines)
  ...
  - Spec file names must map to a real class or module under lib/ (mirror the path). Do not introduce specs for non-existent classes or ad-hoc names (e.g., avoid template_helpers_replacements_spec.rb when testing Rspec::PendingFor::TemplateHelpers; add those examples to template_helpers_spec.rb).
